...
const TestimonialCard: React.FC<{ name: string; role: string; text: string }> = ({ name, role, text }) => (
  <Card className="w-[350px] mx-4 flex-shrink-0 relative p-8 h-full flex flex-col border-gray-800/60 bg-gradient-to-br from-[#1C1C2B] to-[#151522] hover:border-orange-500/50 transition-all duration-300 shadow-xl rounded-[1.5rem]">
    <div className="absolute top-2 right-6 text-6xl text-orange-500/5 font-serif leading-none italic pointer-events-none">“</div>
    ...
    <p className="text-gray-300 mb-8 italic text-sm flex-grow leading-relaxed">"{text}"</p>
    <div className="flex items-center gap-4 pt-6 border-t border-gray-800/50">
      <div className="w-11 h-11 rounded-full bg-gradient-to-tr from-orange-500 to-pink-500 flex items-center justify-center font-bold text-white text-base shadow-inner">
        {name.charAt(0)}
      </div>
      <div>
        <h4 className="text-white font-bold text-sm leading-none mb-1.5">{name}</h4>
        <p className="text-gray-500 text-[11px] uppercase tracking-widest font-semibold">{role}</p>
      </div>
    </div>
  </Card>
);
  ...
  return (
    <section className="py-24 bg-[#10101A] overflow-hidden">
      <div className="text-center mb-16 px-4">
        <span className="inline-block px-4 py-1.5 mb-5 text-[11px] font-bold tracking-[0.2em] text-orange-400 uppercase bg-orange-400/10 rounded-full border border-orange-500/20">
          Social Proof
        </span>
      ...
      <div className="pause-on-hover relative space-y-10">
